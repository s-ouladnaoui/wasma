...
It admits five parameters:
    ...
    3. The max size for a sequence in terms of the number of itemsets composing the sequence:   
                                        if positive we generate sequences where the size is between 1 and s
                                        otherwise the sequence size is fixed to s  
    4. The size of the dataset: represents the number of transations (rows) of the generated dataset
    5. The name of the output file 
    Example
    java ds_generator 100 4 10 1000 ds_100_4_10_1k => generates a random sequence dataset:
                                            over 100 items 
                                            where the itemset length belongs randomly to [1,4]
                                            the number of itemsets per transaction randomly peeked from [1,10]
                                            containing 1k transactions called ds_100_4_10_1k
    java ds_generator 1000 -2 -8 10000 ds_10k => generates a random sequence dataset:
                                            over 1000 items 
                                            where the itemset length is exactly 2 for the entire dataset
                                            the number of itemsets per transaction is fixed to 8
                                            containing 10k transactions
                                            The output file is called ds_10k
*/
